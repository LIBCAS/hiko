            ...
            @foreach ($resources as $index => $resource)
                <div wire:key="resource-{{ $index }}" class="p-3 space-y-6 bg-gray-200 shadow">
                    <div class="required">
                        ...
                        <x-input 
                            wire:model.defer="resources.{{ $index }}.title"
                            name="related_resources[{{ $index }}][title]" 
                            id="resource_title-{{ $index }}"
                            class="block w-full mt-1" type="text" required />
                    </div>
                    <div>
                        <x-label for="resource_link-{{ $index }}" value="URL" />
                        <x-input 
                            wire:model.defer="resources.{{ $index }}.link"
                            name="related_resources[{{ $index }}][link]" 
                            id="resource_link-{{ $index }}"
                            class="block w-full mt-1" type="url" />
                    </div>
