    ...
    <div class="bg-white p-6 shadow rounded">
            ...
            <div class="overflow-x-auto border rounded">
                <table class="w-full text-sm text-left">
                    <thead class="bg-gray-50 text-gray-600 uppercase">
                        <tr>
                            <th class="px-4 py-3">{{ __('hiko.category') }}</th>
                        ...
                        @foreach($issues as $issue)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-right">
                                    @can($issue['type'] === 'global' ? 'manage-users' : 'manage-metadata')
                                    <a href="{{ route(($issue['type'] === 'global' ? 'global.' : '') . $routePrefix . '.category.edit', $issue['id']) }}"
                                       target="_blank"
        ...
        @else
            <div class="text-center py-8 bg-gray-50 rounded border border-dashed border-gray-300">
                <p class="text-gray-500 italic">{{ $hasScanned ? __('hiko.validation_no_issues') : __('hiko.validation_not_started') }}</p>
            </div>
        @endif
    </div>
</div>
