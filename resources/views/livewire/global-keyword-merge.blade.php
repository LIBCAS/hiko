<div x-data="{ showConfig: true, confirmOpen: false }">
    <div x-show="showConfig" x-transition class="bg-white shadow rounded-lg p-6 mb-6 relative">
        ...
        <p class="text-sm text-gray-600 mb-4">{{ __('hiko.global_keyword_merging_criteria_description') }}</p>
    ...
    <div class="bg-white shadow overflow-hidden border border-gray-200 sm:rounded-lg">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        {{ __('hiko.local_keyword') }}
                    </th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        {{ __('hiko.method') }} & {{ __('hiko.reason') }}
                    </th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        {{ __('hiko.global_keyword') }}
                    </th>
                </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($previewData as $item)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 whitespace-nowrap align-top">
                            <input type="checkbox" wire:model.live="selectedIds" id="keyword-{{ $item['local']->id }}" value="{{ $item['local']->id }}" class="rounded border-gray-300 text-primary focus:ring-primary">
                        </td>
                        <td class="px-4 py-3 align-top">
                            <div class="text-sm font-medium text-gray-900 flex gap-3">
                                <a :href="`/keywords/{{ $item['local']->id }}/edit`" target="_blank" class="text-primary hover:underline font-semibold px-2">{{ $item['local']->id }}</a>
                                <span>{{ $item['local']->getTranslation('name', app()->getLocale()) }}</span>
                            </div>
                            <div class="text-xs text-gray-500 mt-0.5">{{ optional($item['local']->keyword_category)->getTranslation('name', app()->getLocale()) ?? '—' }}</div>
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap align-top">
                        ...
                        <td class="px-4 py-3 align-top">
                            @if($item['global'])
                                <div class="text-sm text-gray-900 font-medium flex gap-3">
                                    <a :href="`/global-keywords/{{ $item['global']->id }}/edit`" target="_blank" class="text-primary hover:underline font-semibold px-2">{{ $item['global']->id }}</a>
                                    <span>{{ $item['global']->getTranslation('name', app()->getLocale()) }}</span>
                                </div>
                                <div class="text-xs text-gray-500 mt-0.5">{{ optional($item['global']->keyword_category)->getTranslation('name', app()->getLocale()) ?? '—' }}</div>
                            @else
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-8 text-center text-gray-500">{{ __('hiko.no_results') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<x-global-merge-confirm-modal
    wire:key="global-keyword-confirm-{{ md5(json_encode($confirmationItems)) }}-{{ count($selectedIds) }}-{{ $confirmationMoreCount }}"
    show="confirmOpen"
