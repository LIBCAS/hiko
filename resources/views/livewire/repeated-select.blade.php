<div>
    <div class="p-2 space-y-3 border rounded-md border-primary-light">
        <p>
            {{ $fieldLabel }}
        </p>
        @error('{{ $fieldKey }}')
        ...
        @foreach ($items as $item)
            <div wire:key="{{ $loop->index }}" class="flex">
                <x-select name="{{ $fieldKey }}[]" class="block w-full mt-1" aria-label="{{ $fieldLabel }}"
                    x-data="ajaxChoices({url: '{{ route($route) }}', element: $el, change: (data) => { $wire.changeItemValue({{ $loop->index }}, data) } })" x-init="initSelect()">
                    @if (!empty($item['value']))
