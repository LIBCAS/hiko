<div class="space-y-6">
    <div class="bg-white p-6 shadow rounded">
        ...
        <p class="text-sm text-gray-600 mb-4">{{ __('hiko.consistency_check_description') }}</p>
        ...
        @if(!empty($issues))
            <div class="overflow-x-auto border rounded">
                <table class="w-full text-sm text-left">
                    <thead class="bg-gray-50 text-gray-600 uppercase">
                        <tr>
                            <th class="px-4 py-3">{{ __('hiko.profession') }}</th>
                    ...
                    <tbody class="divide-y">
                        @foreach($issues as $issue)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3">{{ __('hiko.' . $issue['type']) }}</td>
                                <td class="px-4 py-3 text-red-600">{{ $issue['error'] }}</td>
                                <td class="px-4 py-3 text-right">
                                    <a href="{{ $issue['type'] === 'global' ? route('global.professions.edit', $issue['id']) : route('professions.edit', $issue['id']) }}"
                                       target="_blank"
            ...
            <div class="mt-4 text-sm text-gray-500">{{ __('hiko.total_issues_found', ['count' => count($issues)]) }}</div>
