            ...
            <div class="overflow-x-auto border rounded">
                <table class="w-full text-sm text-left">
                    <thead class="bg-gray-50 text-gray-600 uppercase">
                        <tr>
                            <th class="px-4 py-3 border-b">{{ __('hiko.type') }}</th>
                            <th class="px-4 py-3 border-b">{{ __('hiko.identity') }}</th>
                            <th class="px-4 py-3 border-b">{{ __('hiko.validation_error') }}</th>
                            <th class="px-4 py-3 border-b text-right">{{ __('hiko.action') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y">
                        @foreach($issues as $issue)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3">
                                    <span class="px-2 py-1 text-xs font-semibold rounded {{ $issue['type'] === 'global' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800' }}">
                                        {{ $issue['type'] === 'global' ? __('hiko.global') : __('hiko.local') }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 font-medium">
                                    {{ $issue['name'] }}
                                    <span class="text-xs text-gray-400 block">ID: {{ $issue['id'] }}</span>
                                </td>
                                <td class="px-4 py-3 text-red-600">
                                    {{ $issue['error'] }}
                                </td>
                                <td class="px-4 py-3 text-right">
                                    <a href="{{ $issue['type'] === 'global' ? route('global.identities.edit', $issue['id']) : route('identities.edit', $issue['id']) }}"
                                       target="_blank"
