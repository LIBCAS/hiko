        ...
        <div class="bg-white shadow rounded-lg p-6 mb-6">
            ...
            <div class="mt-6 flex justify-center">
                <button wire:click="scan" wire:loading.attr="disabled" class="px-4 py-2 mt-4 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 disabled:opacity-25 disabled:cursor-not-allowed transition ease-in-out duration-150 flex items-center gap-2">
                    <svg wire:loading class="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
        ...
        @else
            <div class="flex flex-col gap-6">
                @foreach($groups as $index => $group)
                    <div wire:key="group-{{ $index }}"
                            x-data="mergeGroupData(@js($group['places']), @js($index))"
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    <template x-for="place in places" :key="place.id">
                                        <tr :class="{'bg-blue-50': selectedIds.includes(place.id)}" class="hover:bg-gray-50">
                                            <td class="px-3 py-3 text-center align-top">
                                                <input type="checkbox" :value="place.id" x-model="selectedIds" class="rounded border-gray-300 text-primary focus:ring-primary">
                                            </td>

                                            <td class="px-3 py-3 align-top text-sm font-mono">
                                                <a :href="`/places/${place.id}/edit`" x-text="place.id" target="_blank" class="text-primary hover:underline"></a>
                                            </td>

                                            <td class="px-3 py-3 align-top cursor-pointer text-sm"
                                                @click="final.name = place.name"
                                                :class="final.name === place.name ? 'bg-yellow-100 font-bold' : ''">
                                                <span x-text="place.name"></span>
                                            </td>

                                            <td class="px-3 py-3 align-top cursor-pointer text-sm"
                                                @click="final.additional_name = place.additional_name"
                                                :class="final.additional_name === place.additional_name ? 'bg-yellow-100 font-bold' : ''">
                                                <span x-text="place.additional_name || '—'"></span>
                                            </td>

                                            <td class="px-3 py-3 align-top cursor-pointer text-sm"
                                                @click="final.country = place.country"
                                                :class="final.country === place.country ? 'bg-yellow-100 font-bold' : ''">
                                                <span x-text="place.country"></span>
                                            </td>

                                            <td class="px-3 py-3 align-top cursor-pointer text-sm"
                                                @click="final.division = place.division"
                                                :class="final.division === place.division ? 'bg-yellow-100 font-bold' : ''">
                                                <span x-text="place.division || '—'"></span>
                                            </td>

                                            <td class="px-3 py-3 align-top cursor-pointer text-xs font-mono"
                                                @click="final.latitude = place.latitude"
                                                :class="final.latitude === place.latitude ? 'bg-yellow-100 font-bold' : ''">
                                                <span x-text="place.latitude || '—'"></span>
                                            </td>

                                            <td class="px-3 py-3 align-top cursor-pointer text-xs font-mono"
                                                @click="final.longitude = place.longitude"
                                                :class="final.longitude === place.longitude ? 'bg-yellow-100 font-bold' : ''">
                                                <span x-text="place.longitude || '—'"></span>
                                            </td>

                                            <td class="px-3 py-3 align-top cursor-pointer text-xs"
                                                @click="final.alternative_names = place.alternative_names"
                                                :class="JSON.stringify(final.alternative_names) === JSON.stringify(place.alternative_names) ? 'bg-yellow-100 font-bold' : ''">
                                                <span x-show="place.alternative_names && place.alternative_names.length > 0">...</span>
                                                <span x-show="!place.alternative_names || place.alternative_names.length === 0">—</span>
                                            </td>

                                            <td class="px-3 py-3 align-top cursor-pointer text-xs"
                                                @click="final.note = place.note"
                                                :class="final.note === place.note ? 'bg-yellow-100 font-bold' : ''">
                                                <span x-show="place.note">...</span>
                                                <span x-show="!place.note">—</span>
                                            </td>

                                            <td class="px-3 py-3 align-top cursor-pointer text-xs font-mono"
                                                @click="final.geoname_id = place.geoname_id"
                                                :class="final.geoname_id === place.geoname_id ? 'bg-yellow-100 font-bold' : ''">
                                                <span x-text="place.geoname_id || '—'"></span>
                                            </td>
                                        </tr>
                                    </template>
...
@script
<script>
    Alpine.data('mergeGroupData', (places, groupIndex) => ({
        places: places,
        ...
        get targetId() {
            ...
            // Filter places to only checked ones
            const checkedPlaces = this.places.filter(p => this.selectedIds.includes(p.id));
            if (checkedPlaces.length === 0) return null;

            // Sort by created_at (string comparison works for ISO dates, fallback to ID if equal)
            checkedPlaces.sort((a, b) => {
                if (a.created_at === b.created_at) return a.id - b.id;
                return (a.created_at < b.created_at) ? -1 : 1;
            });

            return checkedPlaces[0].id;
        },
        ...
        init() {
            ...
            this.$watch('selectedIds', () => {
                this.allChecked = this.selectedIds.length === this.places.length;
            });
        },

        setFinalToTarget() {
            if (this.places.length > 0) {
                // Default to oldest place
                const t = this.places[0];
                this.final = {
                    name: t.name,
        ...
        toggleAll() {
            this.selectedIds = this.allChecked ? this.places.map(p => p.id) : [];
        },

        submitMerge() {
            if (this.selectedIds.length < 2) return;
            if (!confirm('{{ __('hiko.confirm_merge') }}')) return;

            const payload = {
                target_id: this.targetId,
                source_ids: this.selectedIds.filter(id => id !== this.targetId),
