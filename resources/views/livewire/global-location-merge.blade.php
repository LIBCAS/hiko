<div x-data="{ showConfig: true, confirmOpen: false }">
    ...
    <div x-show="showConfig" x-transition class="bg-white shadow rounded-lg p-6 mb-6 relative">
        ...
        <p class="text-sm text-gray-600 mb-4">{{ __('hiko.global_location_merging_criteria_description') }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {{-- Strict Type Match --}}
            <label class="flex items-start p-3 border rounded hover:bg-gray-50 cursor-pointer">
                <input type="checkbox" wire:model.live="criteria" value="type" class="mt-1 mr-2 rounded border-gray-300 text-primary" disabled checked>
                <div>
    ...
    <div class="mb-6 p-4 bg-gray-100 rounded-md border border-gray-200 flex flex-wrap gap-4 items-end">
        ...
        <div>
            ...
            <select wire:model.live="filters.type" class="text-sm border-gray-300 rounded-md shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50 w-40">
                ...
                @foreach(\App\Models\Location::types() as $type)
                    <option value="{{ $type }}">{{ __("hiko.{$type}") }}</option>
                @endforeach
            </select>
        </div>
            ...
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        {{ __('hiko.local_location') }}
                    </th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        {{ __('hiko.method') }} & {{ __('hiko.reason') }}
                    </th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        {{ __('hiko.global_location') }}
                    </th>
                </tr>
            </thead>
                ...
                @forelse($previewData as $item)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 whitespace-nowrap align-top">
                            <input type="checkbox" wire:model.live="selectedIds" id="location-{{ $item['local']->id }}" value="{{ $item['local']->id }}" class="rounded border-gray-300 text-primary focus:ring-primary">
                        </td>
                        <td class="px-4 py-3 align-top">
                            <div class="text-sm font-medium text-gray-900 flex gap-3">
                                <a :href="`/locations/{{ $item['local']->id }}/edit`" target="_blank" class="text-primary hover:underline font-semibold px-2">{{ $item['local']->id }}</a>
                        ...
                        <td class="px-4 py-3 align-top">
                            @if($item['global'])
                                <div class="text-sm text-gray-900 font-medium flex gap-3">
                                    <a :href="`/global-locations/{{ $item['global']->id }}/edit`" target="_blank" class="text-primary hover:underline font-semibold px-2">{{ $item['global']->id }}</a>
                                    <span>{{ $item['global']->name }}</span>
                                </div>
                                <div class="text-xs text-gray-500 mt-0.5">{{ __("hiko.{$item['global']->type}") }}</div>
                            @else
...
<x-global-merge-confirm-modal
    wire:key="global-location-confirm-{{ md5(json_encode($confirmationItems)) }}-{{ count($selectedIds) }}-{{ $confirmationMoreCount }}"
    show="confirmOpen"
