<div>
    ...
    <div class="bg-white shadow rounded-lg p-6 mb-6">
        ...
        <p class="text-sm text-gray-600 mb-4">{{ __('hiko.local_profession_merging_criteria_description') }}</p>
    ...
    <div class="mb-4 flex items-center justify-between">
        <button wire:click="resetScan" class="text-sm text-gray-600 hover:text-gray-900 flex items-center gap-1">← {{ __('hiko.change_criteria') }}</button>
    </div>

    @if(empty($groups))
    <div class="bg-white p-8 text-center rounded border border-dashed border-gray-300">{{ __('hiko.no_duplicates_found') }}</div>
    @else
    <div class="flex flex-col gap-6">
        @foreach($groups as $index => $group)
        <div wire:key="group-{{ $index }}-{{ count($group['items']) }}" x-data="mergeProfessionGroupData(@js($group['items']), @js($index))" class="bg-white shadow rounded-lg overflow-hidden border border-gray-200">
            <div class="bg-gray-50 px-4 py-3 border-b border-gray-200 flex justify-between items-center">
                <h3 class="font-bold text-gray-700">{{ __('hiko.group') }} #{{ $index + 1 }} <span class="ml-2 text-xs font-normal text-gray-500">{{ __('hiko.reason') }}: {{ $group['reason'] }}</span></h3>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                        <tr class="bg-gray-50 text-xs text-gray-500 uppercase">
                            <th class="px-3 py-2 text-center w-10"><input type="checkbox" x-model="allChecked" @change="toggleAll" class="rounded border-gray-300 text-primary focus:ring-primary"></th>
                            ...
                            <th class="px-3 py-2 text-left">{{ __('hiko.identities') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        <template x-for="item in items" :key="item.id">
                            <tr :class="{'bg-blue-50': selectedIds.includes(item.id)}" class="hover:bg-gray-50">
                                <td class="px-3 py-3 text-center align-top"><input type="checkbox" :value="item.id" x-model="selectedIds" class="rounded border-gray-300 text-primary focus:ring-primary"></td>
                                <td class="px-3 py-3 align-top text-sm font-mono"><a :href="`/professions/${item.id}/edit`" x-text="item.id" target="_blank" class="text-primary hover:underline"></a></td>
                                <td class="px-3 py-3 align-top cursor-pointer text-sm" @click="final.cs = item.cs" :class="final.cs === item.cs ? 'bg-yellow-100 font-bold' : ''"><span x-text="item.cs"></span></td>
                                <td class="px-3 py-3 align-top cursor-pointer text-sm" @click="final.en = item.en" :class="final.en === item.en ? 'bg-yellow-100 font-bold' : ''"><span x-text="item.en"></span></td>
                                <td class="px-3 py-3 align-top cursor-pointer text-sm" @click="final.profession_category_id = item.profession_category_id" :class="final.profession_category_id === item.profession_category_id ? 'bg-yellow-100 font-bold' : ''"><span x-text="item.profession_category_label"></span></td>
                                <td class="px-3 py-3 align-top text-sm text-gray-500" x-text="item.identities_count || 0"></td>
                            </tr>
                        </template>
            ...
            <div class="bg-gray-50 px-4 py-3 flex justify-between items-center border-t border-gray-200">
                <div class="text-xs text-gray-500">
                    ...
                    <span x-show="selectedIds.length >= 2 && !canMerge" class="text-red-600 font-bold">{{ __('hiko.local_profession_merge_requires_category') }}</span>
                    <span x-show="selectedIds.length >= 2">{{ __('hiko.merged_profession_id') }}: <span class="font-mono font-bold" x-text="targetId"></span></span>
                </div>
                <button @click="submitMerge" :disabled="!canMerge" class="bg-primary text-white px-4 py-2 rounded text-sm hover:bg-black transition disabled:opacity-50 disabled:cursor-not-allowed">{{ __('hiko.merge_selected') }}</button>
            </div>
...
@script
<script>
    Alpine.data('mergeProfessionGroupData', (items, groupIndex) => ({
        items: items,
        ...
        get targetId() {
            ...
            checkedItems.sort((a, b) => {
                if (!a.created_at || !b.created_at || a.created_at === b.created_at) return a.id - b.id;
        ...
        get finalCategoryLabel() {
            const category = this.items.find(i => i.profession_category_id === this.final.profession_category_id);
            return category ? category.profession_category_label : '—';
        },

        get selectedItems() {
            const selectedIdsString = this.selectedIds.map(String);
            return this.items.filter(i => selectedIdsString.includes(String(i.id)));
        },

        get confirmationItems() {
            const resultId = this.targetId;
            const resultUrl = resultId ? `/professions/${resultId}/edit` : null;

            return this.selectedItems.slice(0, {{ (int) config('merge_confirmation.summary_limit', 20) }}).map(item => ({
                local: item.id,
                local_url: `/professions/${item.id}/edit`,
        ...
        get confirmationPreviewItems() {
            return [
                ...
                { label: 'CS', value: this.previewValue(this.final.cs) },
                { label: 'EN', value: this.previewValue(this.final.en) },
                { label: "{{ __('hiko.category') }}", value: this.previewValue(this.finalCategoryLabel) },
            ];
        },

        get selectedHasAnyCategory() {
            return this.selectedItems.some(i => i.profession_category_id !== null && i.profession_category_id !== undefined);
        },

        get canMerge() {
            return this.selectedIds.length >= 2
                && this.selectedHasAnyCategory
                && this.final.profession_category_id !== null
                && this.final.profession_category_id !== undefined;
        },
        ...
        setFinalToTarget() {
            if (this.selectedIds.length > 0) {
                ...
                if (checkedItems.length > 0) {
                    ...
                    this.final = { cs: t.cs, en: t.en, profession_category_id: t.profession_category_id };
                }
            }
        },

        toggleAll() { this.selectedIds = this.allChecked ? this.items.map(i => i.id) : []; },
