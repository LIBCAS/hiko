        ...
        <div class="max-w-sm col-span-1">
            <form x-data="similarItems({ similarNamesUrl: '{{ route('ajax.places.similar') }}', id: '{{ $place->id }}' })" x-init="$watch('search', () => findSimilarNames($data))" action="{{ $action }}" method="post"
                ...
                <div>
                    ...
                    <x-textarea name="additional_name" id="additional_name" rows="3"
                        class="block w-full mt-1" style="min-height: 90px;">{{ old('additional_name', $place->additional_name) }}</x-textarea>
            ...
            @if ($place->id)
                @can('delete-metadata')
                    <form x-data="{ form: $el }" action="{{ route('global.places.destroy', $place->id) }}" method="post"
                        class="w-full mt-8">
                        @csrf
                        @method('DELETE')
