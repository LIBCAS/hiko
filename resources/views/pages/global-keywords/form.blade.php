<x-app-layout :title="$title">
    ...
    <div class="grid-cols-3 grid gap-4 mb-4 space-y-3">
        <div class="max-w-sm">
            <form 
                x-data="similarItems({ similarNamesUrl: '{{ route('ajax.items.similar', ['model' => 'GlobalKeyword']) }}', id: '{{ $keyword ? $keyword->id : null }}' })" 
            ...
            >
                ...
                <div>
                    ...
                    <x-input 
                        ...
                        type="text" 
                        name="cs" 
                        :value="old('cs', $keyword->getTranslation('name', 'cs') ?? null)"
                ...
                <div>
                    ...
                    <x-input 
                        ...
                        type="text" 
                        name="en" 
                        :value="old('en', $keyword->getTranslation('name', 'en') ?? null)"
                        x-on:change="search = $el.value" 
                    />
                    @error('en')
                        <div class="text-red-600">{{ $message }}</div>
                ...
                <div class="required">
                    ...
                    >
                        ...
                        @foreach ($availableCategories as $availableCategory)
                            @if ($availableCategory instanceof \App\Models\GlobalKeywordCategory)
                                <option value="{{ $availableCategory->id }}" 
                                    {{ old('category', $keyword->keyword_category_id ?? null) == $availableCategory->id ? 'selected' : '' }}>
            ...
            @if ($keyword->id)
                @can('delete-metadata')
                    <form x-data="{ form: $el }" action="{{ route('global.keywords.destroy', ['globalKeyword' => $keyword->id]) }}" method="post"
                        class="max-w-sm mt-8">
                        @csrf
                        @method('DELETE')
            ...
            <div class="max-w-sm bg-white p-6 shadow rounded-md">
                @if ($keyword->keyword_category)
                    <h2 class="text-l font-semibold">{{ __('hiko.attached_category') }}: 
                        <a href="{{ route('keywords.category.edit', $keyword->keyword_category->id) }}" class="border-b-2 text-primary-dark border-primary-light hover:border-primary-dark">
                            {{ $keyword->keyword_category->name }}
                        </a>
                    </h2>
                @else
