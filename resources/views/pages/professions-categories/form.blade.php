    ...
    <div class="grid-cols-3 grid gap-4 mb-4 space-y-3">
        ...
        @if ($professionCategory->id)
            <div class="max-w-sm bg-white p-6 shadow rounded-md">
                ...
                <h2 class="text-l font-semibold">{{ __('hiko.attached_persons_count') }}: {{ $professionCategory->identities->count() }}</h2>
            ...
            <div class="max-w-sm bg-white p-6 shadow rounded-md">
                @if ($professionCategory->professions->count() > 0)
                    <h2 class="text-l font-semibold">{{ __('hiko.professions') }}: {{ $professionCategory->professions->count() }}</h2>
                    <ul class="list-disc p-3">
                        @foreach ($professions as $profession)
                            <li>
                                <a href="{{ route('professions.edit', $profession->id) }}" class="text-sm border-b text-primary-dark border-primary-light hover:border-primary-dark">{{ $profession->name }}</a>
                            </li>
                        @endforeach
                    </ul>
                @else
                    <h2 class="text-l font-semibold">{{ __('hiko.no_attached_professions') }}</h2>
                @endif
            </div>
        @endif
