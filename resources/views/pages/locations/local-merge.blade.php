<x-app-layout :title="$title">
    ...
    <x-page-lock
        scope="tenant"
        resource-type="location_local_merge"
    ...
    <div x-data="{ showGuide: false }">
        ...
        <div x-show="showGuide" x-transition class="mb-6 bg-blue-50 border border-blue-200 rounded-md p-4 relative" style="display: none;">
            ...
            <ul class="list-disc list-inside text-sm text-blue-700 mt-2">
                <li>{{ __('hiko.local_location_merge_step_1') }}</li>
                <li>{{ __('hiko.local_location_merge_step_2') }}</li>
                <li>{{ __('hiko.local_location_merge_step_3') }}</li>
            </ul>
        </div>

        <livewire:local-location-merge />
    </div>
</x-app-layout>
