<x-app-layout :title="$title">
    ...
    <x-page-lock
        scope="global"
        resource-type="location_global_merge"
    ...
    <div x-data="{ showGuide: false }">
        ...
        <div x-show="showGuide" x-transition class="mb-6 bg-blue-50 border border-blue-200 rounded-md p-4 relative" style="display: none;">
            ...
            <ul class="list-disc list-inside text-sm text-blue-700 mt-2">
                <li>{{ __('hiko.global_location_merge_step_1') }}</li>
                <li>{{ __('hiko.global_location_merge_step_2') }}</li>
                <li>{{ __('hiko.global_location_merge_step_3') }}</li>
            </ul>
        </div>

        <livewire:global-location-merge />
    </div>
</x-app-layout>
