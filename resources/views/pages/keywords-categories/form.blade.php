            ...
            <form x-data="similarItems({ similarNamesUrl: '{{ route('ajax.items.similar', ['model' => 'KeywordCategory']) }}', id: '{{ $keywordCategory->id }}' })" x-init="$watch('search', () => findSimilarNames($data))" action="{{ $action }}" method="post"
                ...
                <div>
                    ...
                    <x-input id="cs" class="block w-full mt-1" type="text" name="cs"
                        x-on:change="search = $el.value" :value="old('cs', $keywordCategory->translations['name']['cs'] ?? null)" />
                    @error('cs')
                ...
                <div>
                    ...
                    <x-input id="en" class="block w-full mt-1" type="text" name="en"
                        x-on:change="search = $el.value" :value="old('cs', $keywordCategory->translations['name']['en'] ?? null)" />
                    @error('en')
                        <div class="text-red-600">{{ $message }}</div>
                    @enderror
                </div>
                        ...
                        @foreach ($keywordCategory->keywords->sortBy('name') as $keyword)
                            <li>
                                <a href="{{ route('keywords.edit', $keyword->id) }}"
                                    class="text-sm border-b text-primary-dark border-primary-light hover:border-primary-dark">{{ $keyword->name }}</a>
                            </li>
                        @endforeach
                    </ul>
                @else
