...
class KeywordsTable extends Component
{
    ...
    public $availableGlobalKeywordsCount = 0;
    public $allGlobalKeywords = [];
    public $selectedKeywordOneDetails = null;
    public $selectedKeywordTwoDetails = null;
    public int $globalKeywordChunkSize = 10;
    public int $globalKeywordChunkCount = 1;
    ...
    public function render()
    {
        ...
        if ($this->globalKeywordSearch) {
            $search = strtolower($this->globalKeywordSearch);

            $globalKeywordsToDisplay = $this->allGlobalKeywords;
        ...
        return [
            ...
            'rows'   => $data->map(function ($kw) {
                ...
                $categoryDisplay = $keyword->keyword_category
                    ? $this->formatCategoryLink($keyword->keyword_category, $kw->source)
    ...
    protected function formatCategoryLink($category, string $source): string
    {
        ...
        $link = $source === 'local'
            ? route('keywords.category.edit', $category->id)
    ...
    {
        // Get keyword categories not used by any keywords
        $potentialOrphans = DB::table("{$tenantPrefix}keyword_categories as kc")
        ...
        // Check that no letter is indirectly referencing the keyword category
        foreach ($potentialOrphans as $category) {
            $hasLetterReferences = DB::table("{$tenantPrefix}keyword_letter as kl")
                ->join("{$tenantPrefix}keywords as k", function($join) use ($category) {
                    $join->on("kl.keyword_id", "=", "k.id")
                        ->where("k.keyword_category_id", "=", $category->id);
                })
                ->exists();

            if (!$hasLetterReferences) {
                $safeToDelete[] = $category->id;
            } else {
                Log::info("[cleanUpOrphanedCategories] Category {$category->id} has letter references, skipping deletion");
            }
        }

        // Delete safe keyword categories
        foreach ($safeToDelete as $id) {
            DB::table("{$tenantPrefix}keyword_categories")->where('id', $id)->delete();
            Log::info("[cleanUpOrphanedCategories] Deleted orphaned keyword category ID: {$id}");
        }

        Log::info("[cleanUpOrphanedCategories] Deleted " . count($safeToDelete) . " orphaned keyword categories");
    }
    ...
    public function openManualMerge()
    {
        ...
        $globalKeywords = GlobalKeyword::with('keyword_category')->get();
        $this->availableGlobalKeywords = $globalKeywords->map(function ($k) {
            return [
                'id' => $k->id,
                'cs' => $k->getTranslation('name', 'cs'),
                'en' => $k->getTranslation('name', 'en'),
                'cs_slug' => Str::slug(strtolower($k->getTranslation('name', 'cs') ?? '')),
                'en_slug' => Str::slug(strtolower($k->getTranslation('name', 'en') ?? '')),
                'category_id' => $k->keyword_category_id,
                'category_name' => optional($k->keyword_category)->getTranslation('name', 'cs') ?? '',
            ];
        })->toArray();
        ...
        $this->allGlobalKeywords = $this->availableGlobalKeywords;
    ...
    public function selectLocalKeyword($id, $chunkCount = null)
    {
        $this->isProcessing = true;
        $this->selectedLocalKeyword = $id;
        $this->selectedGlobalKeyword = null;
        if (!$chunkCount) {
            $this->globalKeywordChunkCount = $chunkCount = 1;
        }
        ...
        if ($local) {
            $csSlug = Str::slug(strtolower($local['cs'] ?? ''));
            $enSlug = Str::slug(strtolower($local['en'] ?? ''));

            $scored = collect($this->allGlobalKeywords)->map(function ($global) use ($csSlug, $enSlug, $local) {
                $csSimilarity = 0;
                $enSimilarity = 0;
                similar_text($csSlug, $global['cs_slug'], $csSimilarity);
                similar_text($enSlug, $global['en_slug'], $enSimilarity);

                return array_merge($global, [
                    'csSimilarity' => round($csSimilarity, 1),
                    'enSimilarity' => round($enSimilarity, 1),
                    'avgSimilarity' => round(($csSimilarity + $enSimilarity) / 2, 1),
                    'categoryMatch' => ($local['category_id'] && $global['category_id']) &&
                        ($local['category_name'] === $global['category_name']),
                ]);
            });

            // Sort by starts-with > category match > similarity
            $prefix = strtolower($csSlug);

            $sorted = $scored->sort(function ($a, $b) use ($prefix) {
                $aStarts = Str::startsWith($a['cs_slug'], $prefix);
                $bStarts = Str::startsWith($b['cs_slug'], $prefix);
            ...
            $this->availableGlobalKeywordsCount = $sorted->count();
            $this->availableGlobalKeywords = $sorted->take($this->globalKeywordChunkSize * $chunkCount)->toArray();
        }
    ...
    private function validatePivotUpdates($tenantPrefix, $pivotRecords)
    {
        ...
        if ($totalRecords === 0) {
            Log::info("[validatePivotUpdates] No updated pivots to validate — skipping.");
            return true;
        }
        ...
        $validationRate = ($validUpdates / $totalRecords) * 100;
    ...
    public function mergeAll()
    {
        ...
        if (!$tenant || empty($tenant->table_prefix)) {
            ...
            $this->dispatch('alert', [
                'type' => __('hiko.error'),
                'message' => __('hiko.tenant_prefix_not_found'),
            ]);
            $this->isProcessing = false;
            return;
        }
        ...
        if ($localKeywords->isEmpty()) {
            ...
            $this->dispatch('alert', [
                'type' => __('hiko.error'),
                'message' => __('hiko.no_local_keywords_to_merge'),
            ]);
            $this->isProcessing = false;
            return;
        }
        ...
        try {
            ...
            if ($merged > 0) {
                $this->dispatch('alert', [
                    'type' => __('hiko.success'),
                    'message' => "$merged " . __('hiko.keywords_merged_successfully') . ($skipped > 0 ? " ($skipped " . __('hiko.keywords_skipped') . ")" : ""),
                ]);
            } else if ($skipped > 0) {
                $this->dispatch('alert', [
                    'type' => __('hiko.warning'),
                    'message' => __('hiko.no_keywords_merged') . " ($skipped " . __('hiko.keywords_skipped') . ")",
                ]);
            }
            ...
            $this->dispatch('alert', [
                'type' => __('hiko.error'),
                'message' => __('hiko.error_during_merge') . ": " . $e->getMessage(),
            ]);
        }
    ...
    public function loadMoreGlobalKeywords()
    {
        $this->globalKeywordChunkCount++;
        $this->selectLocalKeyword($this->selectedLocalKeyword, $this->globalKeywordChunkCount);
    }
}
